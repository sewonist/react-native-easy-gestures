...
export default class Gestures extends Component {
  ...
  static propTypes = {
    ...
    minLimit: PropTypes.shape({
      x: PropTypes.number,
      y: PropTypes.number,
    }),
    maxLimit: PropTypes.shape({
      x: PropTypes.number,
      y: PropTypes.number,
    }),
  };
  ...
  onDrag(event, gestureState) {
    ...
    if(this.limit) {
      if (left < this.minLimit.x || left > this.maxLimit.x) left = this.dragStyles.left
      if (top < this.minLimit.y || top > this.maxLimit.y) top = this.dragStyles.top
    }
